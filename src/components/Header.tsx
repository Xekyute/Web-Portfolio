...
const Header = () => {
  ...
  return (
    <header className="relative flex items-center sticky top-0 z-50 border-b bg-black/95 backdrop-blur px-4 py-3">
      {/* Top bar (brand + toggle) */}
      <div className="max-w-7xl mx-auto px-4 flex w-full items-center justify-between">
        {/* Brand */}
              ...
              {[
                ...
                <li key={item.label}>
                  <a
                    ...
                    href={item.href}
                    className="block rounded-lg px-4 py-3 text-left text-white/90 hover:bg-emerald-500/15 hover:text-white focus:outline-none"
                    onClick={() => setIsMenuOpen(false)}
                  >
                    {item.label}
                  </a>
